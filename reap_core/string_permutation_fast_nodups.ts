function string_permutation(text: string) {
  ...
  function backtrack() {
    ...
    for (let index = 0; index < sorted.length; index++) {
      ...
      // before next turn check if next char is the same
      // and the previous character is not being used, meaning
      // we've moved horizontally and have finished the permutation of the previous
      if (index > 0 && sorted[index - 1] == sorted[index] && !used[index - 1]) {
        continue;
      }
